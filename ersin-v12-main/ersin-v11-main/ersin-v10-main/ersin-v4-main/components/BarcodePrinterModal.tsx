...
const BarcodePrinterModal: React.FC<BarcodePrinterModalProps> = ({ isOpen, onClose, products }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/60 backdrop-blur-sm animate-fade-in">
      <style>{`
        @media print {
            @page { size: 55mm ${printSettings.labelHeight}mm; margin: 0mm; }
            html, body { margin: 0 !important; padding: 0 !important; width: 55mm !important; background: white !important; visibility: hidden; }
            #print-area { display: block !important; visibility: visible !important; position: absolute; left: 0; top: 0; z-index: 9999; width: 55mm !important; }
            #print-area * { visibility: visible !important; color: black !important; border-color: black !important; }
            .no-print { display: none !important; }
            .label-container { width: 55mm; height: ${printSettings.labelHeight}mm; page-break-after: always; break-after: page; position: relative; background: white; display: block; padding-top: ${printSettings.marginTop}mm; padding-left: ${printSettings.marginLeft}mm; overflow: hidden; }
            .label-content-wrapper { display: flex; flex-direction: column; width: 100%; height: 100%; padding: ${printSettings.paddingY}mm ${printSettings.paddingX}mm; transform: scale(${printSettings.scale / 100}); transform-origin: top left; }
            ...
            .lbl-center { flex: 1; display: flex; align-items: center; justify-content: center; overflow: hidden; }
      ...
      <div className="bg-white dark:bg-slate-900 w-full h-full md:h-[90vh] md:w-[95vw] md:max-w-[1200px] md:rounded-2xl shadow-2xl overflow-hidden flex flex-col no-print">
      ...
      <div id="print-area">
         {Array.from(selectedProductIds).map(id => {
             ...
             return Array.from({ length: qty }).map((_, i) => (
                 <div key={`${id}-${i}`} className="label-container">
                    <div className="label-content-wrapper">
                        <div className="lbl-header">
                            <div className="font-black text-lg">{product.part_code}</div>
                            <div className="border-2 border-black px-1 rounded-sm font-bold text-xs">{product.location}</div>
                        </div>
                        <div className="lbl-center">
                            {barcodeImages[product.id] && <img src={barcodeImages[product.id]} className="max-w-full max-h-full" />}
                        </div>
                        <div className="lbl-footer">
                            <div className="font-bold text-center text-xs">{product.product_name}</div>
                        </div>
                    </div>
                 </div>
             ));
